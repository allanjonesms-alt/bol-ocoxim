...
export const FederalDrawSimulator: React.FC<FederalDrawSimulatorProps> = ({
  ...
  // Exclusively Real Federal tickets from database
  ...
  return (
    <div className="space-y-6" id="federal-draw-simulator-root">
          ...
          <div className="space-y-1.5 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-slate-500 pt-1">
              <span className="flex items-center gap-1.5 text-emerald-700 font-bold">
                ...
                {federalGames.length} bilhete(s) reais confirmados na base de dados
              </span>
              {activeDraw && (
      ...
      {/* Real Tickets Status Bar & Transparency Report Access */}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-black uppercase tracking-wider text-slate-800">
                  Base Exclusiva de Bilhetes Reais:
                </span>
                <span className="bg-emerald-100 text-emerald-900 border border-emerald-300 text-xs font-black px-2.5 py-0.5 rounded-full">
                  {federalGames.length} bilhete(s) confirmado(s)
                </span>
              </div>
              <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                Utiliza com rigor exclusivo bilhetes reais adquiridos e pagos pelos apostadores no banco de dados.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="inline-flex items-center gap-2 text-xs font-extrabold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 px-4 py-2.5 rounded-xl transition-all cursor-pointer shadow-xs hover:shadow-sm"
              title="Abrir página pública com a listagem de todos os bilhetes em ordem numérica crescente"
            >
              <FileText className="w-4 h-4 text-emerald-600" />
              <span>Relatório de Transparência (Ordem Crescente)</span>
              <ExternalLink className="w-3 h-3 text-emerald-600" />
            </Link>
          </div>
        </div>

        {federalGames.length === 0 ? (
          <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-4 text-amber-900 text-xs flex items-start gap-2.5">
            <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
            <div>
              <strong className="block font-bold">Nenhum bilhete real confirmado encontrado no momento.</strong>
              <p className="text-amber-800/90 text-[11px] mt-0.5">
                Os bilhetes são carregados em tempo real diretamente do banco de dados quando os participantes compram bilhetes da Loteria Federal.
              </p>
            </div>
          </div>
        ) : (
          <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 overflow-x-auto py-1">
              ...
              {[...federalGames]
                .sort((a, b) => (a.numbers[0] || 0) - (b.numbers[0] || 0))
                .slice(0, 10)
                .map((g, idx) => (
                  <span
                    key={g.id || idx}
                    className="bg-slate-100 text-slate-800 border border-slate-200 font-mono text-[11px] font-bold px-2 py-0.5 rounded-md shrink-0"
                    title={`Bilhete real de ${g.userName || 'Apostador'}`}
                  >
                    {formatTicketNumber(g.numbers[0])}
                  </span>
                ))}
              {federalGames.length > 10 && (
                <span className="text-[10px] text-slate-400 font-bold shrink-0">
                  +{federalGames.length - 10} bilhetes no relatório
                </span>
              )}
            </div>
            ...
            <Link
              to="/transparencia-sorteio"
              className="text-[11px] font-bold text-emerald-700 hover:underline shrink-0 flex items-center gap-1"
            >
              <span>Ver todos</span>
